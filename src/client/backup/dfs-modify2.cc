...
namespace velox {
	...
	int DFS::upload(std::string file_name, bool is_binary, uint64_t block_size) {
		...
		cout << "Total Chunks : " << total_chunk_num << " , " << file_info.size << endl;
		...
		while (true) {
			...
			uint64_t cur_cli_wbuf_offset = cli_wbuf_offset;
			//uint64_t cur_cli_wbuf_read_sz = cli_wbuf_read_sz;
			cli_wbuf_offset += current_block_size;

			if(cli_wbuf_sz <= cli_wbuf_offset || end >= file_info.size || chunks_per_node[which_server] <= 0){  // If slab is changed or meet eof, then send to block leader
				block.first = slab_name;	
				...
				if(create_slab[which_server]){
					...
					io_ops.pos = cur_cli_wbuf_offset - cli_wbuf_read_sz;
				...
				cout << "Send Write Buffer : " << cli_wbuf_read_sz << " , " << cur_cli_wbuf_offset << " , " << io_ops.block.first << " , " << cn << endl;
				if(chunks_per_node[which_server] <= 0){
					cn = 0;
					i++;
					cli_wbuf_offset = 0;
					which_server = ((file_hash_key % NUM_NODES) + i) % NUM_NODES;
				}
			}

			myfile.read(chunk.data() + is_first_block + cur_cli_wbuf_offset, current_block_size - bool(is_first_block));
