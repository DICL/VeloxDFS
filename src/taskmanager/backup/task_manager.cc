...
//	if(!pthread_mutex_trylock(&(cur_chunk[idx]->lock))){  // Acquire Mutex lock, fill the shared memory buffer 
...
//			pthread_mutex_unlock(&(cur_chunk[idx]->lock));
			return true;
		}
//		pthread_mutex_unlock(&(cur_chunk[idx]->lock));
//	}
...
void task_worker(std::string file, struct logical_block_metadata& lblock_metadata, string _job_id, int _task_id){
	...
	//	cout << "Outer while loop--> processed_file_cnt: " << processed_file_cnt << " replica_num: " << replica_num << endl;

		while(md_index < input_block_num){
	//		cout << "Inner while loop--> md_index: " << md_index << " input block num: " << input_block_num << endl;
			int index_to_copy;
...
//			DistLockStatus rc = get_dist_lock(zh, zk_path.c_str(), isPrimary);
	//		cout << "[" << job_id << "] get_dist_lock start : " << zk_path << endl;
			DistLockStatus rc = get_dist_lock(zh, zk_eof, zk_path, isPrimary, &Stealing);
	//		cout << "[" << job_id << "] get_dist_lock end" << endl;
			if(rc == GET_DIST_LOCK){
	...
	//			cout << "Inner while loop out" << endl;
				break;
